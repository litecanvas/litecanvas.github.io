...
`);
  ...
  (() => {
    ...
    function m(e, _ = {}) {
      let f = e.text, p = e.textsize, b = e.textalign, d = e.textfont, v = { chars: u, first: 33, size: 8 }, n = 1, t = null, h = (a) => {
        n = Math.round(a / t.size);
      }, w = () => console.warn("textalign() has not yet been implemented for pixel font"), g = (a, c, x, l = 3) => {
        let o = x.charCodeAt(0), r = t.chars[o - t.first];
        if (r) for (let s = 0; s < t.size; s++) for (let i = 0; i < t.size; i++) (r[s] | 0) & 1 << i && e.rectfill(a + i * n, c + s * n, n, n, l);
      }, y = (a, c, x, l = 3) => {
        x += "";
        let o = n * t.size;
        for (let r = 0; r < x.length; r++) g(a, c, x[r], l), a += o;
      };
      return { PIXEL_FONT_BASIC: v, textfont: (a) => {
        typeof a == "object" ? (e.def("text", y), e.def("textsize", h), e.def("textalign", w), t = a) : (e.def("text", f), e.def("textsize", p), e.def("textalign", b), t = null, d(a));
      } };
    }
    window.pluginPixelFont = m;
  })();
})();
